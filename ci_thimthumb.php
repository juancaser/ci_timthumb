 ...
 *
 * Copyright 2013 The Juan Who Code <caserjan@gmail.com>
 *
...
if( file_exists(APPPATH. '/config/timthumb.php'))	require_once('timthumb.php');
...
/**
 * The actual timthumb hook for CI
 */
// Let's clear $_GET, we only need some few
unset($_GET);

$pu = parse_url(rtrim(ltrim($_SERVER['REQUEST_URI'],'/'),'/'));
...
// Image path/url
$url = base64_decode(rawurldecode(array_shift($uri)),true);
if(!$url) exit(); // Invald base64

// Get image size settings
$img_set = explode('x',array_shift($uri));
